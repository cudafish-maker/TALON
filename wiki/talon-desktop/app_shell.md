...
- `talon_desktop.theme` applies the centralized operational Qt palettes,
  stylesheet, left navigation rail styling, dialog/form polish, dense table
  behavior, theme variants, and font-scale aware base sizing.
...
- `talon_desktop.sitrep_page.SitrepPage` is the first non-placeholder feature
  ...
  opt-in state.
- Incoming SITREPs render through the dashboard overlay layer on the main
  content area. ROUTINE/PRIORITY alerts auto-dismiss, higher severities require
  acknowledgement, and FLASH/FLASH_OVERRIDE audio remains gated by the
  persisted core audio opt-in setting.
- `talon_desktop.asset_page.AssetPage` replaces the generic Assets placeholder
  with a table, detail panel, create/edit dialog, shared operational map
  placement picker, verification controls, and deletion-request/server-delete
  command wiring.
...
- `talon_desktop.mission_page.MissionPage` replaces the generic Missions
  placeholder with a list/detail page, extended create workflow, AO/route map
  drawing, point pickers, requested asset selection, server approval allocation
  review, and server lifecycle command controls.
- `talon_desktop.chat_page.ChatPage` replaces the generic Chat placeholder with
  searchable/grouped channel and message navigation, composer, channel
  creation, DM creation, server-only delete controls, urgent rendering/blink,
  operator/alert side panel, and the DM security notice.
...
- Major function pages have command-backed workflows and offscreen smoke
  coverage; remaining gaps are release polish and packaging validation.
- Windows package validation follows Linux release-candidate acceptance.
