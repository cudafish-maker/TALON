...
**Current state:** client enrollment is implemented, but the server operator still authenticates by passphrase and uses the sentinel explicitly for server-authored rows. This must be revisited when server-operator enrollment exists:
...
- Do not add a second permanent "SERVER" callsign once real enrollment exists — the sentinel is a bridge for the current server-authorship model only.

---

## DMs vs Group Chat Encryption

- **Target for DMs:** end-to-end encrypted via PyNaCl in Phase 2b — server routes but cannot read.
- **Current DMs:** server-readable; protected by SQLCipher at rest and RNS transport in flight until Phase 2b lands.
- **Group chat:** server-readable by design.
- Message bodies are stored in SQLCipher; SITREP bodies get additional field encryption, while chat/DM bodies do not yet have DM-specific field encryption.
