...
- [x] Build configs: buildozer.spec, pyinstaller-linux/windows.spec, CI workflows, Linux tarball installer with Kivy/SDL graphics defaults and GLX-failure launcher fallback
...
- [x] Verify tests pass with current migrations and protocol (`pytest -q`: 199 passed in the last full local run; 199 tests collected on 2026-04-25)
...
- [x] Client document fetch/cache: synced document metadata can be opened from client mode by fetching plaintext over the persistent sync link, caching an encrypted local blob, and invalidating stale cached files on document updates/deletes
...
### Operators
- [x] Callsign registration (enrollment flow) — server token generation/enrollment UI, client TOKEN:SERVER_HASH paste flow, operator creation, lease renewal, and RNS loopback enrollment are implemented
- [x] Skills: predefined (8 tactical skills, checkbox toggles) + custom (free-text, add/remove); profile fields: display_name + notes; server can edit any operator from Clients screen; client self-edit UI remains deferred beyond Phase 2 core
...
- [x] Notifications on create/delete/linked-record changes to connected clients + server UI — RNS push and domain-event routing are implemented; there is no separate SITREP edit/append workflow in the current product surface
...
### Chat
- [x] Default channels: #flash, #general, #sitrep-feed, #alerts — seeded by `ensure_default_channels()` on first entry
...
### Documents
- [~] Upload/download files — server upload/delete path is complete; client download now fetches synced documents on demand over the persistent sync link, stores a local encrypted cache, and reuses the integrity/macro warning/save flow; client upload remains deferred; current desktop dashboard lacks visible navigation to the registered Documents screen (BUG-085)
- [ ] Large files broadband-only (queue for non-LoRa connections) — deferred beyond Phase 2 core
