...
| `talon/utils/formatting.py` | DONE | shared formatting helpers used by UI flows |
...
| `talon/db/migrations.py` | DONE | schema v15; migrations cover SERVER sentinel, asset/SITREP links, mission workflow, filesystem documents, client meta keys, operator versioning, tombstones, UUID/offline outbox identity, chat redesign fields, asset deletion requests, extended/custom mission fields, and client-pushable messages. `len(MIGRATIONS) == DB_SCHEMA_VERSION` is asserted. |
...
| `talon/sitrep.py` | DONE | `create_sitrep()` field-encrypts body and supports mission/asset links plus pending sync status; `load_sitreps()` decrypts and joins callsign/asset label; `delete_sitrep()` remains server-only by UI policy. Server-authored rows still use the explicit SERVER sentinel until server-operator enrollment replaces it. |
...
| `talon/waypoints.py` | DONE | mission route waypoint creation/loading/deletion helpers; used by mission create and shared map overlays |
...
| `talon/documents.py` | DONE | secure upload/download/delete/list backend plus client-local cache helpers for on-demand document fetches; cached blobs stay encrypted at rest and are evicted on document updates/deletes |
...
| `talon/network/framing.py` | DONE | shared packet send/chunking and chunk reassembly helpers for client/server RNS messages |
| `talon/network/registry.py` | DONE | table-driven sync metadata, allowlists, client-push/offline flags, tombstone order, field transforms, and UI refresh targets |
...
| `talon/network/protocol.py` | DONE | message type constants; `encode()`/`decode()` (UTF-8 JSON); includes explicit `operator_revoked`, coded `operator_inactive`, and on-demand `document_request` / `document_response` message shapes for client document fetches |
| `talon/network/client_components.py` | DONE | helper components for `ClientSyncManager`: identity/enrollment, broadband/LoRa link lifecycle, outbox push/ack handling, record application, tombstone reconciliation, UI notification dispatch with startup badge suppression, local revocation marking, and on-demand document fetch/cache invalidation |
| `talon/network/client_sync.py` | DONE | `ClientSyncManager` now acts as the stable client sync façade; public API keeps the enrollment/sync methods and now also exposes `fetch_document()` for client document retrieval over the persistent sync link; first sync after login is treated as quiet startup hydration |
...
| `talon/server/net_components.py` | DONE | helper components for `ServerNetHandler`: RNS link routing, active-client tracking, record serialization, sync/tombstone reads, push dispatch, revocation packets, validated enroll/sync/heartbeat/client-push flows, and resource-backed document transfer handling |
...
| `talon/app.py` | DONE | TalonApp(MDApp), deferred server screen registration; `on_data_pushed()` active-screen refresh/badge routing supports quiet refreshes for startup hydration; `dispatch_domain_events()` drives network notifications and local badge/refresh updates from the shared service event stream; global theme and font-scale loading/selection notifies built screens; `resolve_local_operator_id()` / `require_local_operator_id()` wrap the shared operator resolver for UI flows; `net_handler` + `client_sync` refs, stopped on exit; DB shutdown delegates to the shared connection wrapper |
| `main.py` | DONE | entry point, KIVY_NO_ENV_CONFIG guard, Kivy window provider pinned to SDL2, Mesa software GL defaulted on Linux, desktop Kivy multisampling disabled for GLX framebuffer compatibility, mode detection |
...
| `talon/ui/screens/main_screen.py` | DONE | programmatic desktop dashboard with topbar quick-nav, asset panel, full map, collapsible side panels, and mission/SITREP panel; map refresh uses shared `MapContext`; asset panel `MAP` picker filters visible asset markers; mission cards select route/operating-area overlays and force associated mission assets visible; topbar theme selector switches/persists Tactical Green vs Readable Dark; windowed desktop summary labels ellipsize instead of wrapping/clipping; Android deferred Phase 4 |
...
| `talon/ui/screens/mission_create_screen.py` | DONE | OPORD-style mission create wizard with mission types, constraints, phases/objectives, asset requests, route/AO drawing, custom resources, and key locations |
| `talon/chat.py` | DONE | `ensure_default_channels()` seeds `#flash`, `#general`, `#sitrep-feed`, `#alerts`; `create_channel()`, `get_or_create_dm_channel()`, `load_channels()`, `load_messages()`, `send_message()` with UUID/sync_status for client outbox, `delete_message()`, `delete_channel()`; no DM E2E yet — SQLCipher + RNS transport only until Phase 2b |
...
| `talon/ui/screens/document_screen.py` | DONE | document list/detail UI; server upload/delete stays local; client download now runs in a worker thread, fetches from the server on demand when needed, and reuses the existing macro warning + save flow. Screen is registered, but desktop dashboard navigation to it is missing (BUG-085). |
...
| `talon/ui/font_scale.py` | DONE | DB-backed global font scale load/save helpers for desktop readability |
| `talon/ui/widgets/font_scale.py` | DONE | reusable font-scale controls for the dashboard UI |
| `talon/ui/kv/login.kv` | SCAFFOLD | passphrase field + unlock button |
| `talon/ui/kv/lock.kv` | SCAFFOLD | lock icon + status message |
| `talon/ui/kv/main.kv` | DONE | minimal KV rule only; `MainScreen._build_desktop_layout()` builds the dashboard programmatically |
...
| `talon/ui/kv/mission_create.kv` | DONE | registration stub; mission create wizard is built programmatically |
...
| `tests/test_attribution_flows.py` | DONE | UI entry-point checks for shared local-operator attribution in asset, mission, and SITREP flows |
| `tests/test_audio_alerts.py` | DONE | opt-in audio setting persistence and generated WAV sanity coverage |
| `tests/test_chat.py` | DONE | chat message UUID/sync-status coverage for client outbox support |
| `tests/test_crypto.py` | DONE | keystore + fields |
| `tests/test_db.py` | DONE | connection + migrations |
| `tests/test_documents.py` | DONE | document sanitization plus client-local cache store/eviction coverage |
| `tests/test_sync.py` | DONE | heartbeat, version logic, upsert (insert/replace/column filtering), client-push integration, revocation-over-sync/client-lock coverage, and synced-document cache invalidation/delete cleanup |
| `tests/test_enrollment.py` | DONE | token lifecycle, operator creation, lease renewal |
| `tests/test_framing.py` | DONE | packet-size boundary, chunk ordering/duplicate/stale-buffer/buffer-cap coverage |
| `tests/test_map_data.py` | DONE | shared map context, mission overlay filtering, and selected-mission asset-union coverage |
| `tests/test_missions.py` | DONE | mission lifecycle version bumps, SITREP links, custom variants, and delete unlink coverage |
| `tests/test_operators.py` | DONE | operator reads, profile/skills updates, local-operator resolution, and client attribution coverage |
| `tests/test_protocol.py` | DONE | protocol version injection plus malformed message validation coverage |
| `tests/test_registry.py` | DONE | synced-table registry sets, metadata, transforms, and unsupported-table rejection coverage |
| `tests/test_revocation.py` | DONE | operator revocation version bump coverage |
...
| `build/pyinstaller-linux.spec` | DONE | KV files, logo, Kivy `data/`, and `kivymd/icon_definitions.py` bundled directly; no KivyMD `md_icons` runtime shim is loaded |
| `build/pyinstaller-windows.spec` | DONE | mirrors Linux Kivy/KivyMD data collection for the Windows desktop matrix |
| `build/install-talon.sh` | DONE | user-local Linux installer packaged into `talon-linux/install.sh`; installs from its own extracted bundle directory by default, no longer scans directories or accepts archive inputs, checks/installs runtime dependencies including GLX/Mesa DRI pieces needed by Kivy/OpenGL, finds `ldconfig` under `/sbin`/`/usr/sbin`, and falls back to common library paths; validates bundled Kivy/KivyMD assets, creates config/data/RNS/document paths, writes a launcher with Kivy/SDL2 plus Mesa software GL defaults, optional smoke test |
| `build/talon-smoke-client.ini` | DONE | packaged-start smoke-test config used by desktop CI and local PyInstaller smoke runs |
| `build/rns-loopback-server-interface.txt` / `build/rns-loopback-client-interface.txt` | DONE | same-machine RNS loopback interface stanzas for enrollment/sync validation |
| `.github/workflows/build-desktop.yml` | DONE | Linux + Windows matrix on tag/main plus manual dispatch; Linux installs `xvfb`/`xauth`/`xclip`, builds under Xvfb with clean PyInstaller, verifies required packaged runtime assets, smoke-starts the bundle with Kivy/SDL graphics env defaults, copies the installer into `dist/talon-linux/install.sh`, packages the tarball, smoke-installs from the extracted `talon-linux/` directory, and publishes `talon-linux.tar.gz` plus checksum on tag pushes |
