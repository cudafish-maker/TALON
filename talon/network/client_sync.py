    ...
    def _trigger_local_lock_check(self) -> None:
        ...
        try:
            ...
            def _dispatch(_dt: float) -> None:
                ...
                engine = getattr(app, "sync_engine", None) if app else None
...
def _notify_ui(table: str, *, badge: bool = True) -> None:
    """
    Schedule a UI refresh on the Kivy main thread for *table*.
    Safe to call from any thread; silently no-ops outside a running Kivy app.
    """
    try:
        from kivy.app import App
        from kivy.clock import Clock
        ...
        def _dispatch(dt: float) -> None:
            ...
            if app and hasattr(app, "on_data_pushed"):
                app.on_data_pushed(table, badge=badge)
