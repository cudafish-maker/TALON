...
from talon.server.net_components import (
    ServerActiveClients,
    ServerLinkRouter,
...
from talon.network.registry import serialise_record_for_wire
...
def _notify_ui(table: str) -> None:
    """Schedule a server UI refresh after a background network write."""
    try:
        from kivy.app import App
        from kivy.clock import Clock

        def _dispatch(dt: float) -> None:
            app = App.get_running_app()
            if app and hasattr(app, "on_data_pushed"):
                app.on_data_pushed(table)
...
class ServerNetHandler:
    ...
    def __init__(self, conn, cfg, db_key: bytes) -> None:
        self._conn = conn
        self._cfg = cfg
        self._db_key = db_key
        ...
        self._ui_dispatcher = ServerUiDispatcher(notify_ui=_notify_ui)
    ...
    def start(self) -> None:
        ...
        from talon.config import get_data_dir
        from talon.crypto.identity import load_or_create_identity
